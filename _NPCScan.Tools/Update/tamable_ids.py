...
def write(output_filename, data_path, locale):
  ...
  print 'Writing tamable rares from %s Wowhead to <%s>...' % (locale, output_filename)
  ...
  for npc_id, npc_data in sorted(npcs.iteritems()):
    ...
    if npc_id in _WORLDMAP_OVERRIDES:
      ...
      print '\tNpc%d - %r: No valid WorldMap ID.' % (npc_id, npc_names[npc_id])
  # Add extra overrides
  ...
  with codecs.open(output_filename, 'w+', encoding='utf_8') as output:
    ...
    for npc_id, worldmap_id in sorted(npc_worldmap_ids.iteritems()):
      ...
      if worldmap_id in worldmap_names:
        comment.append('from %s' % worldmap_names[worldmap_id])
      if comment:
        comment.insert(0, ' --')
      if worldmap_id is None:
        worldmap_id = 'true'
      output.write('\t[ %d ] = %s;%s\n' % (npc_id, worldmap_id, ' '.join(comment)))
    output.write('};')
