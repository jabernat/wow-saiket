...
assert( OutFile:write( "-- AUTOMATICALLY GENERATED BY <_NPCScan.Tools/Tools/UpdateNPCData.lua>!\n" ) );
assert( OutFile:write( "local Tools = select( 2, ... );\n" ) );

-- Write NPC names
assert( OutFile:write( "Tools.NPCNames = {\n" ) );
for _, NpcID in ipairs( SortOrder ) do
...
assert( OutFile:write( "Tools.NPCDisplayIDs = {\n" ) );
for _, NpcID in ipairs( SortOrder ) do
	local DisplayID = DisplayIDs[ NpcID ];
	if ( DisplayID ) then
		assert( OutFile:write( ( "\t[ %d ] = %d;\n" ):format( NpcID, DisplayID ) ) );
	end
end
assert( OutFile:write( "};\n" ) );

-- Write NPC map IDs
assert( OutFile:write( "Tools.NPCMapIDs = {\n" ) );
for _, NpcID in ipairs( SortOrder ) do
...
-- Write NPC path data
assert( OutFile:write( "Tools.NPCPointData = {\n" ) );
for _, NpcID in ipairs( SortOrder ) do
