--[[****************************************************************************
  ...
  * _Clean.Chat.BlizzardCombatLog.lua - Modifies the Blizzard_CombatLog addon. *
  ****************************************************************************]]
...
--[[****************************************************************************
  * Function: _Clean.Chat.BlizzardCombatLog.FCFDockUpdate                      *
...
--[[****************************************************************************
  * Function: _Clean.Chat.BlizzardCombatLog.Truncate                           *
...
do
	...
	function me.Truncate ( Text )
		if ( #Text > MaxFieldLength ) then
			return Text:sub( 1, max( 0, MaxFieldLength - 1 ) )..L.LOG_TRUNCATE_SUFFIX;
		else
			return Text;
		end
	end
...
--[[****************************************************************************
  * Function: _Clean.Chat.BlizzardCombatLog.FilterEvent                        *
...
--[[****************************************************************************
  * Function: _Clean.Chat.BlizzardCombatLog.CombatLogAddEvent                  *
...
--[[****************************************************************************
  * Function: _Clean.Chat.BlizzardCombatLog.CombatLogRefilterUpdate            *
  * Description: Truncates long strings from the combat log.                   *
  ****************************************************************************]]
do
...
do
	_Clean.RegisterAddOnInitializer( "Blizzard_CombatLog", function ()
		...
		TEXT_MODE_A_STRING_1 = L.LOG_FORMAT;
		wipe( EVENT_TEMPLATE_FORMATS );
		EVENT_TEMPLATE_FORMATS[ "ENCHANT_APPLIED" ] = L.LOG_FORMAT_ENCHANT;
		EVENT_TEMPLATE_FORMATS[ "ENCHANT_REMOVED" ] = L.LOG_FORMAT_ENCHANT;
		for _, Prefix in ipairs( Prefixes ) do
			EVENT_TEMPLATE_FORMATS[ Prefix.."_MISSED" ] = L.LOG_FORMAT_MISS;
		end
		for _, Suffix in ipairs( Suffixes ) do
			EVENT_TEMPLATE_FORMATS[ "ENVIRONMENTAL"..Suffix ] = L.LOG_FORMAT_ENVIRONMENTAL;
		end
		...
		for _, Prefix in ipairs( Prefixes ) do
			for _, Suffix in ipairs( Suffixes ) do
				...
				local Label = L.LogActions[ SuffixLabels[ Suffix ] ];
		...
		for Event, Key in pairs( SpecialLabels ) do
			_G[ "ACTION_"..Event ] = L.LogActions[ Key ];
		end
		...
		local function AddMissTypes ( Event )
			for Type, Key in pairs( MissTypes ) do
				_G[ "ACTION_"..Event..Type ] = L.LogActions[ Key ];
			end
		end
		for _, Prefix in ipairs( Prefixes ) do
		...
		for _, Suffix in ipairs( Suffixes ) do
			for Type, Key in pairs( EnvironmentTypes ) do
		 		_G[ "ACTION_ENVIRONMENTAL"..Suffix..Type ] = L.LogActions[ Key ];
			end
		end


		-- Result labels (i.e. crit/partial block/crush)
		for Type, Label in pairs( L.LogResults ) do
