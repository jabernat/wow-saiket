...
1. Define route data in-game:
   ...
   b. Create a blank path in the desired zone, named with a format
      "Overlay:{NPC ID}:{NPC Name}[:{Unique ID}]".  Unique ID values only
      separate different paths used by mobs (i.e. "1", "2", etc.).
...
class Overlay(object):
  ...
  def __init__(self, map_area_id, npc_id, npc_name):
    self.map_area_id, self.npc_id, self.npc_name = map_area_id, npc_id, npc_name
  ...
  def write_obj(self, output):
    ...
    output.write('o Map Area {:d} - Npc {:d}: {:s}'.format(  # Object name
      self.map_area_id, self.npc_id, self.npc_name))
...
def write(output_path, data_path, routes_filename):
  ...
  with wowdata.mpq.open_dbc_mpq(data_path, _MPQ_LOCALE) as archive:
    with dbc.DBC(archive.open('DBFilesClient/WorldMapArea.dbc'),
      'id', None, None, 'filename') as map_areas \
    :
      ...
      # Join routes by NPC
      map_area_overlays = collections.defaultdict(dict)
      for map_area_name, routes in wowdata.lua.load_saved_variables(
        routes_filename)['RoutesDB']['global']['routes'].items() \
      :
        map_area_id = map_areas.filenames[map_area_name.decode('utf_8')].int('id')
        ...
        for name in routes.keys():
          ...
          if match:  # Path is an overlay face
            ...
            if npc_id not in map_area_overlays[map_area_id]:
              map_area_overlays[map_area_id][npc_id] = Overlay(map_area_id, npc_id, match.group('name'))
            map_area_overlays[map_area_id][npc_id].add_face(
              _route_to_face(routes[name].route))

      # Write one *.obj for each NPC's routes
      for map_area_id, npc_overlays in sorted(map_area_overlays.iteritems()):
        map_folder = 'maparea-{:d}'.format(map_area_id)
